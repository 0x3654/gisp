...
- Docker Engine + Docker Compose Plugin (версия 2.20+) и Git. На Ubuntu/Debian:
  ...
  ```
- 6 ГБ RAM (8 ГБ рекомендовано) и ~6 ГБ свободного диска (образы ≈ 3 ГБ, модель ≈ 1 ГБ; стартовый snapshot временно скачивается отдельным контейнером и после bootstrap очищается автоматически).
