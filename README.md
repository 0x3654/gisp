...
**Требования**
- Docker Engine + Docker Compose Plugin (версия 2.20+) и Git. На Ubuntu/Debian:
  ```bash
  sudo apt update
  sudo apt install -y docker.io docker-compose-plugin git
  sudo usermod -aG docker "$USER"
  ```
- 6 ГБ RAM (8 ГБ рекомендовано) и ~10 ГБ свободного диска (образы + модель ≈ 2 ГБ; после восстановления стартового дампа файл можно удалить и освободить ~850 МБ).
...
Скрипт установит Docker/Compose/Git, клонирует репозиторий, подготовит конфиги, загрузит стартовый дамп через `starter-dump` и запустит контейнеры `postgres_registry`, `api`, `import`, `semantic`, `openwebui`. После завершения откройте http://localhost:3000
...
6. *(опционально)* Загрузите стартовый дамп (reestr + load_log + semantic_items) через готовый контейнер:
   ```bash
   docker compose run --rm --profile starter starter-dump
   ```
...
> Одноразовый образ, который загружает стартовый дамп в базу.
